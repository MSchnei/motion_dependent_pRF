...
expName = 'motDepPrf_testRun'  # set experiment name here
expInfo = {
...
filename = os.path.join(strPathParentUp, 'Conditions',
                        'Conditions_TestRun_run' + str(expInfo['run']) +
                        '.npz')
...
filename = os.path.join(strPathParentUp, 'MaskTextures',
                        'Textures_TestRun.npz')
npzfile = np.load(filename)
stimTexture1 = npzfile["stimTexture1"].astype('int8')
ctrlTexture1 = npzfile["ctrlTexture1"].astype('int8')
stimTexture2 = npzfile["stimTexture2"].astype('int8')
ctrlTexture2 = npzfile["ctrlTexture2"].astype('int8')
stimTexture3 = npzfile["stimTexture3"].astype('int8')
ctrlTexture3 = npzfile["ctrlTexture3"].astype('int8')
...
texTimeBlank = np.tile(np.repeat(np.array([0, 0]),
                                 cfg.nFrames/(cfg.cycPerSec[0]*2)),
                       cfg.cycPerSec[0]*2).astype('int8')
texTimeFlicker = np.tile(np.repeat(np.array([0, 1]),
                                   cfg.nFrames/(cfg.cycPerSec[0]*2)),
                         cfg.cycPerSec[0]*2).astype('int8')
texTimeExpd = np.tile(np.arange(cfg.nFrames/cfg.cycPerSec[0]),
                      cfg.cycPerSec[0]*2).astype('int8')
texTimeCntr = np.tile(np.arange(cfg.nFrames/cfg.cycPerSec[0])[::-1],
                      cfg.cycPerSec[0]*2).astype('int8')

texTimeFlicker2 = np.tile(np.repeat(np.array([0, 1]),
                                    cfg.nFrames/(cfg.cycPerSec[1]*2)),
                          cfg.cycPerSec[1]*2).astype('int8')
texTimeExpd2 = np.tile(np.arange(cfg.nFrames/cfg.cycPerSec[1]),
                       cfg.cycPerSec[1]*2).astype('int8')
texTimeCntr2 = np.tile(np.arange(cfg.nFrames/cfg.cycPerSec[1])[::-1],
                       cfg.cycPerSec[1]*2).astype('int8')
...
while clock.getTime() < totalTime:
    ...
    # get key for texture
    keyTexture = conditions[i, 1]

    # blank
    if keyMask == 0 and keyTexture == 0:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 0])
        # get mask to define the opacity values (background)
        if power:
            opaPgDnMask = np.squeeze(opaPgDnMasks[:, :, 0])
        # set timing for the opacity
        visOpa = cycTransp
        # set timing sequence for the texture
        texTime = texTimeBlank
        # set texture
        visTexture = ctrlTexture1

    # cirlce and static/flicker control
    elif keyMask == 1 and keyTexture == 1:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 3])
        # get mask to define the opacity values (background)
        if power:
            opaPgDnMask = np.squeeze(opaPgDnMasks[:, :, 3])
        # set timing for the opacity
        visOpa = cycAlt
        # set timing sequence for the texture
        ...
        # set texture
        visTexture = ctrlTexture1
    # cirlce and contracting motion
    elif keyMask == 1 and keyTexture == 2:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 3])
        # get mask to define the opacity values (background)
        if power:
        ...
        texTime = texTimeCntr
        # set texture
        visTexture = stimTexture1
    # cirlce and expanding motion
    elif keyMask == 1 and keyTexture == 3:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 3])
        # get mask to define the opacity values (background)
        if power:
    ...
    # 3 wedges and static/flicker control
    elif keyMask == 2 and keyTexture == 1:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
            opaPgDnMask = np.squeeze(opaPgDnMasks[:, :, 1])
        # set timing for the opacity
        visOpa = cycAlt
        # set timing sequence for the texture
        texTime = texTimeFlicker
        # set texture
        visTexture = ctrlTexture1
    # 3 wedges and contracting motion
    elif keyMask == 2 and keyTexture == 2:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        # set texture
        visTexture = stimTexture1
    # 3 wedges and expanding motion
    elif keyMask == 2 and keyTexture == 3:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
    ...
    # 3 wedges, 60 deg shifted and static/flicker control
    elif keyMask == 3 and keyTexture == 1:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 2])
        # get mask to define the opacity values (background)
        if power:
            opaPgDnMask = np.squeeze(opaPgDnMasks[:, :, 2])
        # set timing for the opacity
        visOpa = cycAlt
        # set timing sequence for the texture
        texTime = texTimeFlicker
        # set texture
        visTexture = ctrlTexture1
    # 3 wedges, 60 deg shifted and contracting motion
    elif keyMask == 3 and keyTexture == 2:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 2])
        # get mask to define the opacity values (background)
        if power:
        ...
        # set texture
        visTexture = stimTexture1
    # 3 wedges, 60 deg shifted and expanding motion
    elif keyMask == 3 and keyTexture == 3:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 2])
        # get mask to define the opacity values (background)
        if power:
    ...
    # 3 wedges, double speed and static/flicker control
    elif keyMask == 4 and keyTexture == 1:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        visOpa = cycAlt
        # set timing sequence for the texture
        texTime = texTimeFlicker2
        # set texture
        visTexture = ctrlTexture2
    # 3 wedges, double speed and contracting motion
    elif keyMask == 4 and keyTexture == 2:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        visOpa = cycAlt
        # set timing sequence for the texture
        texTime = texTimeCntr2
        # set texture
        visTexture = stimTexture2
    # 3 wedges, double speed and expanding motion
    elif keyMask == 4 and keyTexture == 3:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        visOpa = cycAlt
        # set timing sequence for the texture
        texTime = texTimeExpd2
        # set texture
        visTexture = stimTexture2

    # 3 wedges, double frequency and static/flicker control
    elif keyMask == 5 and keyTexture == 1:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        texTime = texTimeFlicker
        # set texture
        visTexture = ctrlTexture3
    # 3 wedges, double frequency and contracting motion
    elif keyMask == 5 and keyTexture == 2:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
        ...
        texTime = texTimeCntr
        # set texture
        visTexture = stimTexture3
    # 3 wedges, double frequency and expanding motion
    elif keyMask == 5 and keyTexture == 3:
        # get mask to define the opacity values (foreground)
        opaPgUpMask = np.squeeze(opaPgUpMasks[:, :, 1])
        # get mask to define the opacity values (background)
        if power:
